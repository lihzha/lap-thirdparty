...
def _sleep_backoff(base: int) -> None:
    jitter = random.randint(0, 6)
    print(f"{_ts()} - Next check in {base}s (+{jitter}s jitter)...")
...
class TPUManager:
    ...
    def create(self, version: Literal["v4", "v5", "v6"], *, tpu_num: int, topology: str | None = None) -> bool:
        ...
        common = [
            ...
            self.env.tpu_project,
            "--service-account",
            "irom-service-account@mae-irom-lab-guided-data.iam.gserviceaccount.com",
            "--spot",
        ]
        if version == "v4":
            if not topology:
    ...
    def tail_log(self, version: Literal["v4", "v5", "v6"], *, worker: int = 0) -> int:
        cmd = "cd ~/openpi-cot/logs && tail -n 200 -f $(ls -1t | head -n1)"
        proc = gcloud_tpu_ssh(
            tpu_name=self.env.tpu_name,
            project=self.env.tpu_project,
            zone=self._zone_for(version),
