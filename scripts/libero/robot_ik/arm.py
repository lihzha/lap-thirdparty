...
robot_type = os.environ.get("FRANKA_ROBOT_TYPE", "panda")
...
class FrankaArm(RobotArm):
    def _build(self):
        ...
        chosen_type = robot_type if robot_type in {"panda", "fr3"} else "panda"
        model_file = os.path.join(dir_path, "franka", f"{chosen_type}.xml")
