...
for DATASET in "${DATASETS[@]}"; do
    ...
    tpu v4 "source ~/.zshrc && cd openpi-cot && git checkout main && git pull origin main && uv run --group tpu scripts/vis_oxe_dataset.py pi_combined_cot_v4 --exp-name=vis_dataset_${DATASET} --fsdp-devices=4 --batch-size=16 --data.shuffle-buffer-size=400 --model.max-token-len=180 --model.enable-prediction-training --data.no-use-json-actions --data.data-mix=${DATASET} --model.prompt-format=schema_compact" 2>&1 | tee -a "$LOG_FILE"
