...
class PiFastConfig(_model.BaseModelConfig):
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "PiFast":
        return PiFast(self, rngs=nnx.Rngs(rng))
    ...
    def inputs_spec(self, *, batch_size: int = 1) -> tuple[CoTObservation, _model.Actions]:
        ...
        with at.disable_typechecking():
            observation_spec = CoTObservation(
                ...
                tokenized_loss_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.bool_),
            )
...
class PiFast(_model.BaseModel):
    def __init__(self, config: PiFastConfig, rngs: nnx.Rngs):
        ...
        # Use fake observation to initialize image encoder
    ...
    def embed_inputs(
        ...
        for name in obs.images:
            ...
            # Handle both [B, H, W, C] and [B, T, H, W, C] formats
            if len(image.shape) == 5:
                # Take first frame for FAST model
                image = image[:, 0]
    ...
    def sample_actions(
        ...
        *,
        ...
        """Sample actions following pi0_fast logic."""
        # Preprocess observation using our own API
        observation = preprocess_observation(
            None,
            observation,
